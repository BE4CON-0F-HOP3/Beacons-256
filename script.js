...
document.querySelectorAll('a[href^="#"]').forEach(link => {
  link.addEventListener('click', e => {
    ...
    if (id.length > 1) {
      ...
      if (target) {
        e.preventDefault();
        const headerOffset = 80; // adjusted header height with logo
...
document.querySelectorAll('.fade-up, .logo').forEach(el => observer.observe(el));
...
console.log('Beacon of Hope Foundation • Led by Frank (Logo integrated)');
